...
public class ChatTimeUtil {
    //格式化時間格式仿微信顯示
    public static String createChatShowTime(String currentShowTime){
        try {
            currentShowTime=getNewChatTime(Long.parseLong(currentShowTime));
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public static String getConversation(long timesamp) {
        String result = "";
        Calendar todayCalendar = Calendar.getInstance();
        Calendar otherCalendar = Calendar.getInstance();
        otherCalendar.setTimeInMillis(timesamp);
