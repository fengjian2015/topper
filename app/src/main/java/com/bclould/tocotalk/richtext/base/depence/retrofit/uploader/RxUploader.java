package com.bclould.tocotalk.richtext.base.depence.retrofit.uploader;
...
import com.bclould.tocotalk.richtext.base.depence.retrofit.uploader.utils.UploadHelper;
...
public enum RxUploader {
    ...
    public TaskController upload(String url, String filePath, Long id){
        return upload(url, UploadHelper.DEFAULT_FILE_KEY,filePath,id);
    }
    ...
    public TaskController upload(String url, String mediaType, String parName, File file, final Long id){
        ...
        if(!mTasks.containsKey(uuid) && !mPreparingTasks.contains(UploadSingleTask.getFakeTask(taskController))) {
    ...
    public void removePreparingTask(TaskController controller){
//        Iterator<UploadSingleTask> iterator = mPreparingTasks.iterator();
//        while (iterator.hasNext()){
//            if(iterator.next().getTaskController().equals(controller)) {
//                iterator.remove();
//                break;
//            }
//        }
        mPreparingTasks.remove(UploadSingleTask.getFakeTask(controller));
    }

    public UploadSingleTask getPreparingTask(TaskController controller){
        for (UploadSingleTask mPreparingTask : mPreparingTasks) {
            if (controller.equals(mPreparingTask.getTaskController()))
                return mPreparingTask;
        }
        return null;
    }

    public TaskController getPreparingTask(long id){
        for (UploadSingleTask mPreparingTask : mPreparingTasks) {
            TaskController taskController = mPreparingTask.getTaskController();
            if (taskController.id == id && DEFAULT_TASKS_ID.equals(taskController.parentId))
                return taskController;
        }
        return null;
    }

    public TaskController getPreparingTask(long parentId,long id){
        for (UploadSingleTask mPreparingTask : mPreparingTasks) {
            TaskController taskController = mPreparingTask.getTaskController();
            if (taskController.id == id && parentId ==taskController.parentId)
                return taskController;
        }
        return null;
    }
    ...
    TaskController handle(Long parentId, Long id){
        ...
        if(mTasks.containsKey(uuid))
            return mTasks.get(uuid).getTaskController();
        return getPreparingTask(parentId,id);
    }
   ...
   public @Nullable TaskController handleLatestPreparing(){
    ...
    private void add2TaskQueue(UploadSingleTask task){
        ...
        if(runTaskNum.get() >= maxRunningNum){
            ...
            Log.e("add2TaskQueue", String.valueOf(mPreparingTasks.size()));
        }else {
    ...
    private void pull2UploadQueue(){
        Log.e("pull2UploadQueue", String.valueOf(mPreparingTasks.size()));
        if(!mPreparingTasks.isEmpty()){
    ...
    public class TaskController implements Uploader.UploadTaskInfo {
        ...
        public TaskController receiveEvent(Uploader.OnUploadListener listener){
            int num = 0;
        ...
        public void receiveEvent(Uploader.OnUploadListener listener, int filterNum){
            if(mTasks.containsKey(getUUID())){
                mTasks.get(getUUID()).receiveEvent(listener,filterNum);
            }else{
                UploadSingleTask task = getPreparingTask(this);
                if(task != null) {
                    task.prepareReceiveEvent(listener, filterNum);
                }
            }
        }
        ...
        public void stopReceive(){
            if(mTasks.containsKey(getUUID())){
                ...
                UploadSingleTask task = getPreparingTask(this);
                if(task != null) {
                    task.stopReceiveEvent();
                    //task.setOnTaskAutoFinishedListener(null);
                }
            }
        }
